...
class CUHKPEDES(BaseDataset):
    ...
    def __init__(self, root='', verbose=True):
        ...
        self.parsed_cap = read_json('./data/cuhk_peds_parsed_captions.json')

        if False:
            save_list = []
            for index, data in enumerate(self.train):
                save_list.append({"caption_id": index, "caption": data[3]})
                # if index == 100:
                #     break
            write_json(save_list, './data/cuhk_peds_captions.json')
